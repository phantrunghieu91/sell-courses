export default class JinsAccordion {
  /**
   * Initialize accordion components on the page
   */
  constructor() {
    this.accordions = document.querySelectorAll('.jins-accordion');
   ...
   * a custom event ('jins_accordion:toggle') when a button is clicked.
   *
   * @param {HTMLElement} accordion - The root element of the accordion component.
   */
  initAccordion(accordion) {
    const buttons = accordion.querySelectorAll('.jins-accordion__button');
    const panels = accordion.querySelectorAll('.jins-accordion__panel');

    // bind custom event listeners
    accordion.addEventListener('jins_accordion:toggle', event => {
    ...
    buttons.forEach(button => {
      button.addEventListener('click', () => {
        const toggleEvent = new CustomEvent('jins_accordion:toggle', {
          detail: {
            button: button,
            buttons: buttons,
            panels: panels
  ...
  changeIcon(button, expanded) {
    ...
    if (icon) {
      icon.textContent = expanded ? icon.dataset.collapsedCode : icon.dataset.expandedCode;
    }
  }
}
